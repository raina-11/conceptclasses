...
1. An authenticated administrator reserves an import, uploads one `.xlsx` file to the private bucket, confirms the upload, and invokes the parsing Edge Function.
2. The function validates and normalizes the workbook into Postgres. It records a staged, quarantined, or failed terminal state and then idempotently deletes the raw object.
...
6. Set both functions' exact allowed browser origin and deploy both with JWT verification left enabled:
   ...
   Supabase supplies the functions' platform credentials. Do not copy a service credential to Netlify or the SPA. `student-account` must remain server-side: its JWT check authenticates the caller and its service-only RPCs recheck authorization before using Auth Admin operations.
...
- [ ] Migrations were dry-run and applied before both Edge Functions; `QPT_ALLOWED_ORIGINS` and `STUDENT_ACCOUNT_ALLOWED_ORIGINS` are exact; JWT verification remains enabled for both functions; and no service/secret key is present in Netlify or browser code.
